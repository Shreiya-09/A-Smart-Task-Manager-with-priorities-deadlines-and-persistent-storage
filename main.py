...
def add_task(tasks):
    ...
    task = {
        "id": new_id ,"title": title,"description": description,"deadline": deadline,"priority": priority,"status": "pending"}
    tasks.append(task)
    print(" Task added.")
    return tasks
...
def view_tasks(tasks):
    if not tasks:
        ...
        for task in tasks:
            print(f'ID: {task["id"]} | Title: {task["title"]} |Priority: {task.get("priority","N/A")} | Deadline: {task["deadline"]} | Status: {task["status"]}')
...
def edit_task(tasks):
    ...
    for task in tasks:
        if task["id"] == task_id:
            ...
            if field in ["title", "description", "deadline", "status","priority"]:
